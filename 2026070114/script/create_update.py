...
4. 如果版本目录已存在，会将新的暂存区文件合并到该版本目录，同名文件自动覆盖
"""
...
class UpdateCreator:
    ...
    def get_staged_files(self):
        ...
        try:
            ...
            # 过滤掉 update_apply.txt 文件和 data 目录下的所有内容 (保留.sql文件)
            staged_files = [
                f for f in staged_files 
                if f != 'update_apply.txt' 
                and (not f.startswith('data/') or f.endswith('.sql'))
                and f != '.gitignore' 
                and f != '.env' 
            ]
    ...
    def create_update_package(self, version=None, skip_git_check=False):
        ...
        if version_dir.exists():
            print(f"检测到已存在的版本目录：{version_dir}")
            print("将合并新的暂存区文件到现有版本目录，同名文件会覆盖旧文件")
...
def main():
    ...
    parser.add_argument('version', nargs='?', help='版本号（格式：YYYYMMDDHH），如果目录存在则合并暂存区文件并覆盖同名文件')
    ...
    if args.version:
        ...
        if version_dir.exists():
            if args.skip_git:
                print(f"版本目录 {args.version} 已存在，跳过暂存区检查并重新生成file_list后推送")
                success = creator.regenerate_existing_version(args.version)
            else:
                print(f"版本目录 {args.version} 已存在，将合并新的暂存区文件并覆盖同名文件")

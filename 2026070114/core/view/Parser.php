...
class Parser
{
    ...
    public static function compile($tplPath, $tplFile)
    {
        ...
        // 解析文件包含，需要优先解析
        self::$content = self::parseIncludeContent(self::$content, $tplFile);
     ...
     *            当前模板内容
     * @param string $currentTplFile
     *            当前模板文件物理路径
     * @param int $depth
     *            当前递归层级
     * @return string
     */
    private static function parseIncludeContent($content, $currentTplFile, $depth = 0)
    {
        if ($depth >= 50) {
        ...
        if (preg_match_all($pattern, $content, $matches)) {
            ...
            $count = count($arr);
            for ($i = 0; $i < $count; $i ++) {
                $inc_file = self::resolveIncludeFilePath($brr[$i], $currentTplFile);
                file_exists($inc_file) ?: error('包含文件不存在！' . $inc_file);

                $inc_content = file_get_contents($inc_file);
                if ($inc_content === false) {
                    error('包含的模板文件' . $brr[$i] . '读取错误！');
                } else {
                    $inc_content = self::parseIncludeContent($inc_content, $inc_file, $depth + 1);
                    $content = str_replace($arr[$i], $inc_content, $content); // 包含内容
     ...
     * @return string
     */
    private static function resolveIncludeFilePath($includeFile, $currentTplFile)
    {
        // 绝对路径 include 仍然保持历史兼容。
        if (strpos($includeFile, '/') === 0) {
            return ROOT_PATH . $includeFile;
        }

        if (! ! $pos = strpos($includeFile, '@')) {
            return APP_PATH . '/' . substr($includeFile, 0, $pos) . '/view/' . basename(self::$tplPath) . '/' . substr($includeFile, $pos + 1);
        }

        $relativeFile = self::normalizeFilePath(dirname($currentTplFile) . '/' . $includeFile);
        if (file_exists($relativeFile)) {
            return $relativeFile;
        }

        $fallbackFile = self::normalizeFilePath(self::buildThemeIncludeFallbackPath($includeFile));
    ...
    /**
     * 构建主题目录级别的 include 回退路径，兼容旧模板写法。
     *
     * @param string $includeFile
     *            include 中声明的文件路径
     * @return string
     */
    private static function buildThemeIncludeFallbackPath($includeFile)
    {
        $is_home_theme_path = M == 'home';
        if (! $is_home_theme_path && defined('IS_STATIC_GENERATE')) {
            $tpl_dir = Config::get('tpl_dir');
            $home_tpl_root = is_array($tpl_dir) && isset($tpl_dir['home']) ? ROOT_PATH . $tpl_dir['home'] : ROOT_PATH . '/template';
            $current_tpl_path = self::normalizeFilePath(self::$tplPath);
            $home_tpl_root = self::normalizeFilePath($home_tpl_root);
            $is_home_theme_path = strpos($current_tpl_path, rtrim($home_tpl_root, '/')) === 0;
        }

        if ($is_home_theme_path) {
            $htmldir = Config::get('tpl_html_dir') ? trim(Config::get('tpl_html_dir'), '/\\') . '/' : '';
            return rtrim(self::$tplPath, '/\\') . '/' . $htmldir . $includeFile;
        }

        return rtrim(self::$tplPath, '/\\') . '/' . $includeFile;
    }

    /**
     * 标准化文件路径分隔符，避免双斜杠导致判断异常。
     *
     * @param string $path
     *            需要标准化的路径
     * @return string
     */
    private static function normalizeFilePath($path)
    {
    ...
    private static function parOutputArrVal()
    {
        ...
        self::$content = preg_replace_callback($pattern, function($matches) {
            if (isset($matches[3]) && $matches[3]) {
                return "<?php echo @\$this->vars['{$matches[1]}']['{$matches[2]}']['{$matches[4]}'];?>";
            } else {
                return "<?php echo @\$this->vars['{$matches[1]}']['{$matches[2]}'];?>";
            }
        }, self::$content);
    }

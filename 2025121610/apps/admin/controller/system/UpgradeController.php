...
class UpgradeController extends Controller
{
     ...
     * 执行已下载的 SQL 迁移文件
     */
    public function executeDownloadedSql()
    {
        if (!$_POST) {
            json(0, '请求方式错误');
        }
     ...
     * 判断是否为补丁中的 SQL 迁移文件
     */
    private function isSqlPatchFile($file)
    {
        return substr($file, -4) === '.sql' && strpos($file, 'data/') === 0;
    }

    /**
     * 按类型拆分补丁文件
     */
    private function splitPatchFilesByType($files)
    {
        $sqlFiles = array();
        $otherFiles = array();

        foreach ($files as $file) {
            if ($this->isSqlPatchFile($file)) {
                $sqlFiles[] = $file;
            } else {
                $otherFiles[] = $file;
            }
    ...
    private function executeSql($version)
    {
        ...
        // 2. 筛选出 .sql 文件
        $patchFiles = $this->splitPatchFilesByType($files);
        $sqlFiles = $patchFiles['sql'];

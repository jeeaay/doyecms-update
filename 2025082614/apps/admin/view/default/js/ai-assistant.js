...
var AiAssistant = {
    ...
    optimizeBeforeSubmit: function() {
        ...
        if (!content) {
            layer.msg('未找到可优化的内容', {icon: 2});
            return;
        }
        
        // 创建优化任务列表（按执行顺序排列）
        var tasks = [
            { type: 'description', name: 'SEO描述', field: 'textarea[name="description"]', content: content },
            { type: 'title', name: '标题优化', field: 'input[name="title"]', content: content },
            { type: 'subtitle', name: '副标题生成', field: 'input[name="subtitle"]', content: content },
            { type: 'url', name: 'URL优化', field: 'input[name="filename"]', content: content },
            { type: 'keywords', name: 'SEO关键词', field: 'input[name="keywords"]', content: content },
            { type: 'tags', name: '标签生成', field: 'input[name="tags"]', content: content }
        ];
        ...
        var progressHtml = '<div class="ai-optimize-progress">' +
            '<div class="progress-header">' +
                ...
                '<p style="margin: 5px 0; color: #666; font-size: 12px;">正在按顺序执行优化任务...</p>' +
            '</div>' +
            '<div class="progress-content">' +
                '<ul class="task-list">';
        
        tasks.forEach(function(task) {
            progressHtml += '<li class="task-item" data-task="' + task.type + '">' +
        ...
        var progressIndex = layer.open({
            ...
            success: function() {
                // 开始分步执行优化任务
                self.executeSequentialOptimization(content, tasks, 0, {});
            }
        });
    },
     ...
     */
    executeSequentialOptimization: function(content, tasks, currentIndex, context, retryCount) {
        var self = this;
        ...
        if (self.shouldSkipTask(currentTask, context)) {
            ...
            // 继续下一个任务
            setTimeout(function() {
                self.executeSequentialOptimization(content, tasks, currentIndex + 1, context, retryCount);
            }, 200);
            return;
        }
        ...
        function(error) {
            ...
            if (retryCount[taskKey] <= 3) {
                ...
                // 延迟后重试当前任务
                setTimeout(function() {
                    self.executeSequentialOptimization(content, tasks, currentIndex, context, retryCount);
                }, 1000); // 重试前等待1秒
            } else {
                // 重试次数用完，任务最终失败
    ...
    buildPromptForTask: function(task, content, context) {
        var baseContent = '标题：' + ($('input[name="title"]').val() || '未设置') + '\n\n正文：' + content;
        ...
        switch(task.type) {
            ...
            case 'subtitle':
                var optimizedTitle = context.title || $('input[name="title"]').val() || '';
                var seoDesc = context.description || '';
            ...
            case 'url':
                var optimizedTitle = context.title || $('input[name="title"]').val() || '';
                return '请为以下内容，生成SEO友好的URL（英文，用连字符分隔）直接返回内容不要解释：\n\n标题：' + optimizedTitle + '\n\n正文：' + content;
    ...
    checkTaskStatus: function(taskId, isPageLoad, successCallback, errorCallback) {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.code == 1) {
                    // 修复：正确解析嵌套的状态结构
                    var statusObj = response.data.status;
                    var status = statusObj ? statusObj.status : null;
                    ...
                    if (status === 'completed') {
                        ...
                        // 任务失败
                        self.clearTask();
                        var error = response.data.error || statusObj.message || '任务处理失败';
                        if (errorCallback) {
                            errorCallback(error);
                        } else if (isPageLoad) {

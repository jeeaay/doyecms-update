<?php
/*
 * @file            apps/admin/controller/IndexController.php
 * @description     默认主页
 * @author          jeay <wrjie@msn.cn>
 * @createTime      2025-06-10 09:07:07
 * @lastModified    2025-12-16 09:09:28
...
class IndexController extends Controller
{
    ...
    public function home()
    {
        ...
        if (get_db_type() == 'sqlite') {
            // 数据库配置含有默认名字则进行修改
            if (strpos($this->config('database.dbname'), 'doyecms') !== false) {
                if (get_user_ip() != '127.0.0.1' && $this->modDB()) { // 非本地测试时尝试自动修改数据库名称
                    $dbsecurity = true;
                } else {
                    $dbsecurity = false;
                }
            } elseif (file_exists(ROOT_PATH . '/data/doyecms.db')) { // 存在多余的默认数据库文件则改名
                rename(ROOT_PATH . '/data/doyecms.db', ROOT_PATH . '/data/' . get_uniqid() . '.db');
            }
        } elseif (file_exists(ROOT_PATH . '/data/doyecms.db')) {
            rename(ROOT_PATH . '/data/doyecms.db', ROOT_PATH . '/data/' . get_uniqid() . '.db');
        }

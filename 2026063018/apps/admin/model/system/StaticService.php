...
class StaticService extends Controller
{
    ...
    public function generateSort($scode, $allPages = true)
    {
        ...
        // 单页模式
        if ($sort->type == 1) {
            return $this->generateAbout($sort, $allPages);
        } 
        // 列表模式
        else {
     ...
     * @return string|false
     */
    private function generateAbout($sort, $allPages = true)
    {
        // 渲染单页
        ...
        $firstPage = $this->renderAboutPage($sort, $data, 1);
        $pageCount = $allPages ? max(1, intval($firstPage['page_count'])) : 1;
        $firstPath = '';

        for ($page = 1; $page <= $pageCount; $page++) {
            $pageData = $page === 1 ? $firstPage : $this->renderAboutPage($sort, $data, $page);
            $path = $this->getStaticPath($pageData['url']);
    ...
    private function generateList($sort, $allPages)
    {
        ...
        $firstPage = $this->renderListPage($sort, 1);
        $pageCount = $allPages ? max(1, intval($firstPage['page_count'])) : 1;
        $firstPath = '';

        for ($page = 1; $page <= $pageCount; $page++) {
            $pageData = $page === 1 ? $firstPage : $this->renderListPage($sort, $page);
            $path = $this->getStaticPath($pageData['url']);
            $this->writeHtml($path, $pageData['content']);
            if (!$firstPath) {
                $firstPath = $path;
     ...
     * @return array
     */
    private function renderListPage($sort, $page)
    {
     ...
     * @return array
     */
    private function renderAboutPage($sort, $data, $page)
    {
        $snapshot = $this->applyStaticPagingContext($sort, $page);
        try {
            $this->resetPagingViewVars();
    ...
    private function getStaticPath($url)
    {
        ...
        // 拼接完整物理路径
        // 格式：基础路径 + static_dir + default_lang + url
        // 迁移后使用 /www 作为基础路径，未迁移使用 DOC_PATH
        $basePath = defined('WWW_MIGRATED') ? ROOT_PATH . '/www' : DOC_PATH;
        return $basePath . $this->staticDir . '/' . $this->defaultLang . $url;
    }

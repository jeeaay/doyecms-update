...
{
    $cip = get_trusted_client_ip();
    if ($cip === '') {
        $cip = normalize_client_ip_value(isset($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR'] : '');
    }
...
/**
 * 按配置读取可信代理头中的真实客户端 IP。
 *
 * @return string
 */
function get_trusted_client_ip(): string
{
    if (! Config::get('client_ip.trusted_proxy_enable')) {
        return '';
    }
    ...
    $headers = Config::get('client_ip.trusted_headers', true);
    if (! is_array($headers) || ! $headers) {
        return '';
    }

    foreach ($headers as $header) {
        $serverKey = build_trusted_ip_server_key($header);
        if (! $serverKey || ! isset($_SERVER[$serverKey])) {
            continue;
        }

        $clientIp = normalize_client_ip_value($_SERVER[$serverKey]);
        if ($clientIp !== '') {
            return $clientIp;
        }
    }
...
{
    $secret = trim((string) Config::get('client_ip.trusted_proxy_auth_secret'));
    if ($secret === '') {
        return true;
    }

    $header = Config::get('client_ip.trusted_proxy_auth_header');
    $serverKey = build_trusted_ip_server_key($header);
    if ($serverKey === '' || ! isset($_SERVER[$serverKey])) {
        return false;
    }

    $actual = trim((string) $_SERVER[$serverKey]);
    if ($actual === '') {
        return false;
    }
...
/**
 * 将配置中的头名转成 $_SERVER 可读取的键名。
 *
 * @param string $header 头名
 * @return string
 */
function build_trusted_ip_server_key($header): string
{
...
/**
 * 规范化客户端 IP 字符串，仅接受合法 IPv4/IPv6。
 *
 * @param string $value 原始 IP 值
 * @return string
 */
function normalize_client_ip_value($value): string
{

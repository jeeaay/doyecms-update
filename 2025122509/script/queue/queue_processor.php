...
class AiQueueProcessor
{
    ...
    private function processTask($task)
    {
        ...
        try {
            ...
            switch ($taskData['type']) {
                ...
                case 'spec_translate':
                    $result = $this->processSpecTranslate($taskData);
                    break;
    ...
    private function handleTaskFailure($taskId, $taskData, $errorMessage)
    {
        ...
        if (isset($taskData['type']) && $taskData['type'] === 'spec_translate') {
            $this->updateSpecTranslateStatusOnFailure($taskData, $errorMessage);
        }
    }
     ...
     * @throws Exception
     */
    private function processSpecTranslate($taskData)
    {
        if (! isset($taskData['data']) || ! is_array($taskData['data'])) {
            throw new Exception('spec_translate 任务数据格式错误！');
        }

        $data = $taskData['data'];

        $specId = isset($data['spec_id']) ? (int) $data['spec_id'] : 0;
        $sourceFile = isset($data['source_file']) ? $data['source_file'] : '';
        $outputFile = isset($data['output_file']) ? $data['output_file'] : '';
        $targetLang = isset($data['target_lang']) ? $data['target_lang'] : '';
        $baseLanguage = isset($data['base_language']) ? $data['base_language'] : '';

        if (! $specId || ! $sourceFile || ! $outputFile || ! $targetLang) {
            throw new Exception('spec_translate 任务参数不完整！');
        }

        if (! is_file($sourceFile)) {
            throw new Exception('源HTML文件不存在：' . $sourceFile);
        }

        $htmlContent = @file_get_contents($sourceFile);
        if ($htmlContent === false) {
            throw new Exception('读取源HTML文件失败：' . $sourceFile);
        }

        $prompt = $this->buildSpecTranslatePrompt($htmlContent, $baseLanguage, $targetLang);
        $translatedHtml = $this->callAiApi($prompt);
        ...
        if (! check_dir(dirname($outputFile), true)) {
            throw new Exception('创建翻译文件目录失败：' . dirname($outputFile));
        }

        $result = @file_put_contents($outputFile, $translatedHtml);
        if ($result === false) {
            throw new Exception('写入翻译文件失败：' . $outputFile);
        }

        try {
            $model = new SpecPageModel();
            $model->upsertTranslationRecord($specId, $targetLang, basename($outputFile), '1', '');
        } catch (Exception $e) {
            $this->log('更新专题翻译成功状态失败：' . $e->getMessage(), 'ERROR');
        }
     ...
     * @return string
     */
    private function buildSpecTranslatePrompt($htmlContent, $baseLanguage, $targetLang)
    {
        $baseLanguageText = $baseLanguage ?: '源语言';
        $targetLanguageText = $targetLang;

        $prompt = "你是一名专业的网站本地化工程师。\n";
        $prompt .= "现在有一整页HTML代码，需要将其中所有“用户可见的文本内容”从{$baseLanguageText}翻译为{$targetLanguageText}对应的语言。\n";
        $prompt .= "请严格遵守以下要求：\n";
        $prompt .= "1. 必须完整保留原始HTML结构、标签和属性，不要新增或删除标签。\n";
        $prompt .= "2. 不要修改任何class、id、href、src、data-* 等属性。\n";
        $prompt .= "3. 只翻译页面中用户可以看到的文本内容，包括标题、段落、按钮、链接文字等。\n";
        $prompt .= "4. 不要翻译HTML标签名和属性名。\n";
        $prompt .= "5. 直接返回翻译后的完整HTML代码，不要添加任何解释或额外文本。\n\n";
        $prompt .= "下面是需要翻译的HTML：\n\n";
        $prompt .= $htmlContent;
     ...
     */
    private function updateSpecTranslateStatusOnFailure($taskData, $errorMessage)
    {
        if (! isset($taskData['data']) || ! is_array($taskData['data'])) {
            return;
        }
        ...
        if (! $specId || ! $targetLang) {
            return;
        }

        try {
            $model = new SpecPageModel();
            $model->upsertTranslationRecord($specId, $targetLang, basename($outputFile), '2', $errorMessage);
        } catch (Exception $e) {
            $this->log('更新专题翻译失败状态失败：' . $e->getMessage(), 'ERROR');
        }
    }
}

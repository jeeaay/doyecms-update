...
class SpecPageStaticService
{
    ...
    public function generateSpecPageHtml($specId)
    {
        ...
        $parsedContent = $this->parseTemplateContent($templateContent, $specRootPath);
    ...
    private function buildTemplateFullPath($templatePath)
    {
        ...
        $fullPath = DOC_PATH . '/' . $relativePath;
     ...
     * @throws Exception
     */
    private function parseTemplateContent($templateContent, $specRootPath, $depth = 0)
    {
        $maxDepth = 5;
        if ($depth > $maxDepth) {
        ...
        foreach ($matches as $match) {
            ...
            $includeFilePath = $this->buildIncludeFilePath($includePath, $specRootPath);
            ...
            $includeContent = $this->parseTemplateContent($includeContent, $specRootPath, $depth + 1);
     ...
     * @return string
     */
    private function buildIncludeFilePath($includePath, $specRootPath)
    {
        $cleanPath = ltrim($includePath, '/');

        if (strpos($cleanPath, 'spec/') === 0) {
            return DOC_PATH . '/' . $cleanPath;
        }

        return rtrim($specRootPath, '/') . '/' . $cleanPath;
    }

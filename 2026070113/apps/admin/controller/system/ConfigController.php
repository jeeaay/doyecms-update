...
class ConfigController extends Controller
{
    ...
    public function index()
    {
        ...
        if ($_POST) {
            ...
            if (post('submit') === 'accesskey') {
                $this->prepareTranslateConfigPost();
            }
            foreach ($_POST as $key => $value) {
                if (! preg_match('/^[\w\-]+$/', $key)) {
                    continue;
                }
                if ($key === 'TranslateAccessKeySecret' && trim((string) post($key)) === '') {
                    continue;
                }
                $config = array(
                    'debug',
    ...
    private function prepareTranslateConfigPost()
    {
        $translateUrl = trim((string) post('TranslateUrl'));
        if ($translateUrl === '') {
            $translateUrl = 'https://t.dourry.cn/';
        }
        $translateUrl = $this->ensureUrlTrailingSlash($translateUrl);
        if (! filter_var($translateUrl, FILTER_VALIDATE_URL)) {
            error('翻译接口地址格式不正确！', -1);
        }
        $_POST['TranslateUrl'] = $translateUrl;

        $accessKeyId = trim((string) post('TranslateAccessKeyId'));
        $accessKeySecret = trim((string) post('TranslateAccessKeySecret'));
        $existingSecret = (string) $this->config('TranslateAccessKeySecret');

        if ($accessKeySecret !== '' && $accessKeyId === '') {
            error('请先填写翻译接口AccessKeyId，再填写AccessKeySecret！', -1);
        }

        if ($accessKeyId !== '' && $accessKeySecret === '' && $existingSecret === '') {
            error('请填写翻译接口AccessKeySecret！', -1);
        }
    }

    private function applyTranslateConfigViewData(&$configs)
    {
        if (! isset($configs['TranslateUrl'])) {
            $configs['TranslateUrl'] = $this->createVirtualConfigItem('TranslateUrl', 'https://t.dourry.cn/');
        } else {
            $configs['TranslateUrl']['value'] = $configs['TranslateUrl']['value'] ?: 'https://t.dourry.cn/';
        }

        if (! isset($configs['TranslateAccessKeyId'])) {
            $configs['TranslateAccessKeyId'] = $this->createVirtualConfigItem('TranslateAccessKeyId', '');
        }

        if (! isset($configs['TranslateAccessKeySecret'])) {
            $configs['TranslateAccessKeySecret'] = $this->createVirtualConfigItem('TranslateAccessKeySecret', '');
        }

        $secretValue = isset($configs['TranslateAccessKeySecret']['value']) ? (string) $configs['TranslateAccessKeySecret']['value'] : '';
        $configs['TranslateAccessKeySecret']['masked_value'] = $this->maskSensitiveValue($secretValue);
    }

    private function createVirtualConfigItem($name, $value)
    {
        return array(
            'name' => (string) $name,
            'value' => (string) $value,
    ...
    private function ensureUrlTrailingSlash($url)
    {
        $url = trim((string) $url);
        if ($url === '') {
            return $url;

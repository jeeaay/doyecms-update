...
class UpgradeController extends Controller
{
    ...
    private function executeSql($version)
    {
        ...
        foreach ($files as $file) {
            if (substr($file, -4) === '.sql' && strpos($file, 'data/') === 0) {
                $sqlFiles[] = $file;
            }
        }
        ...
        if (!$dbConfig || $dbConfig['type'] !== 'sqlite') {
             // 目前只支持 SQLite 自动备份迁移，MySQL 暂不处理或需另行实现
             // 如果是MySQL，可以直接执行SQL，但这里主要针对 user requirement: "备份客户端的数据库为原数据库名_年月日时分秒.db.bk"
             if ($dbConfig['type'] !== 'sqlite') {
                 // 简单支持 MySQL 执行 SQL，不备份（或视需求而定）
                 // $this->executeMysqlMigration($sqlFiles);
                 return;
             }
        }

        $dbPath = $dbConfig['dbname'];
        $fullDbPath = ROOT_PATH . $dbPath;

        if (!file_exists($fullDbPath)) {
            throw new Exception('数据库文件不存在：' . $fullDbPath);
        }

        // 4. 备份数据库
        $backupName = str_replace('.db', '_' . date('YmdHis') . '.db.bk', basename($dbPath));
        $backupPath = dirname($fullDbPath) . '/' . $backupName;

        if (!copy($fullDbPath, $backupPath)) {
            throw new Exception('数据库备份失败，无法继续执行迁移。');
        }

        // 5. 执行 SQL
        // 使用 PDO 连接数据库执行
        try {
            $dsn = 'sqlite:' . $fullDbPath;
            $pdo = new \PDO($dsn);
            $pdo->setAttribute(\PDO::ATTR_ERRMODE, \PDO::ERRMODE_EXCEPTION);

            foreach ($sqlFiles as $sqlFile) {
                $sqlFilePath = ROOT_PATH . '/' . $sqlFile;
                if (file_exists($sqlFilePath)) {
                    $sqlContent = file_get_contents($sqlFilePath);
                    if ($sqlContent) {
                        // 支持多条 SQL 语句执行
                        // SQLite 可能不支持一次执行多条，或者 PDO exec 支持。
                        // 通常建议按 ; 分割，但需注意字符串内的 ;
                        // 这里简化处理，尝试直接 exec，如果包含事务需小心
                        $pdo->exec($sqlContent);
                    }
                }
            }
            ...
            // 此时数据库可能处于中间状态。
            // 简单回滚：用备份覆盖回去
            copy($backupPath, $fullDbPath);
            throw new Exception('SQL迁移失败，已回滚数据库。错误信息：' . $e->getMessage());
        }
    }
    ...
    /**
     * 缓存补丁文件清单到本地 update 目录，供完成安装阶段读取。
     *
     * @param string $version 补丁版本号
     * @param array $files 补丁文件列表
     * @return void
     */
    private function cachePatchFileList($version, array $files)
    {
        $patch_dir = ROOT_PATH . '/update/' . $version;
        if (!check_dir($patch_dir, true)) {
            throw new \Exception('无法创建补丁清单目录：' . $patch_dir);
        }

        $file_list_path = $patch_dir . '/file_list.txt';
        $content = implode("\n", $files);
        if ($content !== '') {
            $content .= "\n";
        }

        if (file_put_contents($file_list_path, $content) === false) {
            throw new \Exception('无法写入补丁清单文件：' . $file_list_path);
        }
    }
    ...
    public function getFileList()
    {
        ...
        try {
            ...
            // 将远程文件清单缓存到本地，供 finishInstall 阶段识别补丁中的 SQL 文件。
            $this->cachePatchFileList($version, $files);

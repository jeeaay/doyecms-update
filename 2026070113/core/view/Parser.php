        ...
        if (preg_match_all($pattern, self::$content, $matches)) {
            ...
            for ($i = 0; $i < $count; $i ++) {
                ...
                if (strpos($brr[$i], '/') === 0) {
                    ...
                    // 仅当前模板路径属于前台主题目录时，include 文件才需要带上模板子目录。
                    $is_home_theme_path = false;
                    if (defined('IS_STATIC_GENERATE')) {
                        ...
                        $current_tpl_path = str_replace('\\', '/', self::$tplPath);
                        $home_tpl_root = str_replace('\\', '/', $home_tpl_root);
                        $is_home_theme_path = strpos($current_tpl_path, rtrim($home_tpl_root, '/')) === 0;
                    }
                    if (M == 'home' || $is_home_theme_path) {

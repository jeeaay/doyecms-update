...
class Kernel
{
	...
	private static function GetPathInfo()
	{
		...
		if ($path_info)
		{
			IpBlacklist::trackPath($path_info);
		}
		define('P', $path_info);
		return $path_info;
	}
	...
	private static function StaticHtml()
	{
		...
		$static_dir = getenv('STATIC_GENERATE_DIR') ?: '/html';
		$static_index_filename = getenv('STATIC_INDEX_FILENAME') ?: 'index.html';
		$default_lang = getenv('STATIC_DEFAULT_LANG_DIR') ?: 'default';
		
		// 根据子域名决定静态目录
		$lang = self::resolveStaticDir($default_lang);
		
		$static_root = self::getStaticRootPath();
		$base_dir = $static_root . rtrim($static_dir, '/\\') . '/' . basename($lang);
		if (! is_dir($base_dir))
		{
			$lang = $default_lang;
			$base_dir = $static_root . rtrim($static_dir, '/\\') . '/' . basename($lang);
	...
	/**
	 * 根据子域名解析静态文件目录
	 * @param string $default_lang 默认语言目录
	 * @return string 静态文件目录名
	 */
	private static function resolveStaticDir($default_lang)
	{
		// 获取域名
		$host = get_http_host();
		if (! $host)
		...
		// 获取主域名配置（如 abc.com.cn）
		$main_domain = strtolower(getenv('DOMAIN_NAME') ?: '');
		if (! $main_domain)
		{
			return $default_lang;
		}
		...
		// 多级子域名用 _ 连接（如 m.ru → m_ru）
		return str_replace('.', '_', $subdomain);
	}

	/**
	 * 获取静态文件根目录。
	 */
	private static function getStaticRootPath()
	{
		return rtrim(ROOT_PATH, '/\\') . '/www';
	}

...
class ParserModel extends Model
{
    ...
    // 单个分类信息，不区分语言，兼容跨语言
    public function getSort($scode, $multiplename= false)
    {
        ...
        $field = array(
            ...
            'b.type',
            'b.urlname',
            'b.listtpl AS def_listtpl',
            'b.contenttpl AS def_contenttpl',
            'd.gcode'
        );
        $join = array(
            array(
        ...
        $res = parent::table('ay_content_sort a')->field($field)
...
/*     // 分类子类集
    ...
    // 生成静态时，获取所有内容ID（支持分页）
    public function getAllContentIds($offset = 0, $limit = 100)
    {
        return parent::table('ay_content')
            ->where("outlink=''")
            ->where("status=1") // 仅获取启用状态的内容
            ->order('id ASC')
            ->limit($offset, $limit)
            ->column('id');
    }
    ...
    // 生成静态时，获取所有内容总数
    public function getAllContentCount()
    {
        return parent::table('ay_content')
            ->where("outlink=''")
            ->where("status=1")

    ...
    buildPromptForTask: function(task, content, context) {
        ...
        switch(task.type) {
            ...
            case 'title':
                ...
                var prompt = '请优化以下标题，使其更具吸引力和SEO友好，不超过100个字符，直接返回内容不要解释，纯文本不要带格式，以' + aiLanguage + '回复：\n\n' + baseContent;
                if (seoDesc) {
                    prompt += '\n\n参考SEO描述：' + seoDesc;
                }
                return prompt;
            ...
            case 'subtitle':
                var seoDesc = context.description || '';
                var prompt = '请为以下内容生成副标题，直接返回内容不要解释，不超过30个字符，纯文本不要带格式，以' + aiLanguage + '回复：\n\n标题：' + currentTitle;
                if (seoDesc) {
                    prompt += '\n\nSEO描述：' + seoDesc;
                }
                return prompt;
            ...
            case 'url':
                var seoDesc = context.description || '';
                var prompt = '请为以下内容，生成SEO友好的URL（英文，用连字符分隔，不超过8个单词）直接返回内容不要解释：\n\n标题：' + currentTitle;
                if (seoDesc) {
                    prompt += '\n\nSEO描述：' + seoDesc;
                }
                return prompt;
            ...
            case 'keywords':
                var seoDesc = context.description || '';
                var prompt = '请为以下内容，生成SEO关键词（英文，逗号分隔，不超过10个关键词），直接返回内容不要解释，纯文本不要带格式，以' + aiLanguage + '回复：\n\n标题：' + currentTitle;
                if (seoDesc) {
                    prompt += '\n\nSEO描述：' + seoDesc;
                }
                return prompt;
            ...
            case 'tags':
                var seoDesc = context.description || '';
                var prompt = '请为以下内容生成标签（英文，逗号分隔，不超过10个标签）直接返回内容不要解释，纯文本不要带格式：\n\n标题：' + currentTitle;
                if (seoDesc) {
                    prompt += '\n\nSEO描述：' + seoDesc;
                }
                return prompt;

...
class ParserController extends Controller
{
    ...
    /**
     * 获取当前渲染上下文中的页码，优先读取分页组件注入的模板变量。
     *
     * @return int
     */
    private function getCurrentPageNumber()
    {
        $pagecurrent = intval($this->getVar('pagecurrent'));
        if ($pagecurrent > 0) {
            return $pagecurrent;
        }
...
//    }
    ...
    /**
     * 规范化 IF 条件表达式，供安全检查使用
     *
     * @param string $condition IF 条件表达式
     * @return string
     */
    protected function normalizeIfConditionForSecurityCheck($condition)
    {
        if (! is_string($condition) || $condition === '') {
            return '';
        }

        // 先移除块注释，避免利用注释分割函数名绕过检测。
        ...
        // 清理不可见控制字符，降低混淆绕过风险。
        $condition = preg_replace('/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]+/', '', $condition);
        if ($condition === null) {
            $condition = '';
        }
    ...
    /**
     * 检测 IF 条件中是否存在不安全的函数调用
     *
     * @param string $condition 已规范化的 IF 条件表达式
     * @param array $whiteFun 允许的函数白名单
     * @return bool
     */
    protected function containsUnsafeIfFunctionCall($condition, $whiteFun = array())
    {
        if ($condition === '') {
            return false;
        }

        if (! preg_match_all('/\b([A-Za-z_][A-Za-z0-9_]*)\s*\(/', $condition, $matches)) {
            return false;
        }

        foreach ($matches[1] as $functionName) {
            $functionName = strtolower(trim($functionName));
            if (function_exists($functionName) && ! in_array($functionName, $whiteFun)) {
                return true;
            }
        }
